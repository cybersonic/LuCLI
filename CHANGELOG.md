...
- **HTTPS Connector Fix:** Fixed HTTPS configuration to use modern Tomcat `SSLHostConfig` nested element structure instead of deprecated connector-level SSL attributes. This resolves the "No SSLHostConfig element was found" error when starting servers with HTTPS enabled.
