...
## Unreleased
- **Bundled Module Install Pipeline (`modules-install`):** Added build support for bundling one or more module source directories into `target/modules-install` via `build.sh --module-source <path>` (repeatable), packaging them into the JAR under `/modules-install`, and auto-deploying those bundled modules on first startup into the active profile modules directory (for example `~/.markspresso/modules`).
- **Build-Time Branding Resource Injection:** Added a filtered `branding.properties` resource that can be populated at build time (for example via `-Dbranding.*` Maven properties) and loaded at runtime to drive CLI profile resolution (binary-name match, display name, prompt prefix, home/backups directory names, and banner text) without requiring code changes per branded distribution.
- **Markspresso Binary Profile + Home Isolation:** Added a dedicated `markspresso` CLI profile that activates when invoked as `markspresso` (including path/extension-normalized binary names like `markspresso.sh`), with profile-aware defaults now resolving to `~/.markspresso` and `~/.markspresso_backups` instead of `~/.lucli`.
