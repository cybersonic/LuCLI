...
- **Tomcat Configuration Loading:** Refactored `--include-tomcat-web` and `--include-tomcat-server` flags in `--dry-run` mode to load actual `web.xml` and `server.xml` files from the Lucee Express distribution instead of using template files. XML DOM-based patching is applied to the loaded files to respect `enableLucee` and `enableREST` configuration flags, ensuring the dry-run preview shows the actual configuration that will be used.
